...
from joblib import load #loads the trained model
...
#main fucntion where packets are classsified 
def classifyPackets():
#these are the output coloums of the "exported" CSV from tshark
    ...
    try:
        ...
        with open("outputFile.txt", "w") as file:
            for row in readcsv:
                ...
                if count >= 100: #quit after 10 packets
                    break
        ###########################
    ...
    df["sniDomain"] = df["tls.handshake.extensions_server_name"]   #keep the name before its replaces w arbitrary numerical value
    df["tls.handshake.extensions_server_name"] = (df["tls.handshake.extensions_server_name"].astype("category").cat.codes)   #now encode data farme after being labeled:
    ...
    #formatting a single packet data frame including only the fields we want  to send to the ML
    X = df[["frame.number", "frame.time_relative", "ip.src", "ip.dst", "tls.handshake.extensions_server_name", "frame.len"]]

    model = load("networkingLM7Classifier.joblib")    #load in trained model
    predictions = model.predict(X)  #send it our df and store prediction
    df["predictedLabel"] = predictions

    with open("predictedResults.txt","w") as f:
        f.write(df[["sniDomain", "predictedLabel"]].to_string())
